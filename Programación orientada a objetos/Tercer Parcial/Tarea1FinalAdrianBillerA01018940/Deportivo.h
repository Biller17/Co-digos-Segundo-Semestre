...
protected:
    ...
    int turboCompressor;
    Deportivo(){}
    Deportivo(const Deportivo &A){}
    int tag = 2;

public:
    Deportivo(string man, string mod,int year, string col, int pass, int door, int pr, int kil, bool std, bool us, bool tur, bool com)
    {
        manufacturer = man;
        model = mod;
        year = year;
    ...
    void printChar()
    {
        ...
        if(turbo)
        {
            cout<<"Auto con turbo"<<endl;
            cout<<"Turbo compressor de :"<<turboCompressor<<endl;
        }
        else
        {
            cout<<"Auto sin turbo"<<endl;
        }
    }
    int getAge()
    {
